...
public class HibToPng extends Configured implements Tool {

  public static class HibToPngMapper extends Mapper<HipiImageHeader, ByteImage, BooleanWritable, Text> {
    ...
    public void setup(Context context) throws IOException {
      ...
      path = new Path(conf.get("pngfromhib.outdir"));
    ...
    public void map(HipiImageHeader header, ByteImage image, Context context) throws IOException, InterruptedException {
      ...
      PngCodec.getInstance().encodeImage(image, os);
      os.flush();
      os.close();
  ...
  public int run(String[] args) throws Exception {
    ...
    // Check arguments
    /*if (args.length != 2) {
      System.out.println("Usage: hibToPng.jar <input HIB> <output directory>");
      System.exit(0);
    }*/
    ...
    conf.setStrings("pngfromhib.outdir", outputPath);

    //Configuration conf = getConf();

    // Setup MapReduce classes
    Job job = Job.getInstance(conf, "pngfromhib");
    job.setJarByClass(HibToPng.class);
    job.setMapperClass(HibToPngMapper.class);
    ...
    job.addFileToClassPath(new Path("habib/hipi-dependency-jars/hipi-2.1.0.jar")); 
    job.addFileToClassPath(new Path("habib/hipi-dependency-jars/json-simple-1.1.1.jar"));
    job.addFileToClassPath(new Path("habib/hipi-dependency-jars/metadata-extractor-2.9.1.jar"));
    job.addFileToClassPath(new Path("habib/hipi-dependency-jars/xmpcore-5.1.2.jar"));
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new HibToPng(), args);
    System.exit(res);
  }
}
