...
SCRIPT="cd; cd /usr/local/spark-2.1.0/; sbin/stop-mesos-shuffle-service.sh; cd"

echo "**********Stopping Mesos Shuffle Service**********"

for HOSTNAME in ${HOSTS} ; do
    ssh -o StrictHostKeyChecking=no -l ${USERNAME} ${HOSTNAME} "${SCRIPT}"
    echo "**********Mesos Shuffle Service Stopped on" ${HOSTNAME}"**********" 
done

echo ""
echo "==================================================="
echo "SUCCESS: Mesos shuffle service stopped on all nodes"
echo "==================================================="
