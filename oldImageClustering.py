...
#              images data from HDFS and writes it's output to HDFS. Output is a
...
if __name__ == "__main__":
    ...
    images_rdd = sc.binaryFiles(input_path) \
        ...
        .map(lambda x: (Row(fileName=x[0], features=x[1].tolist())))

    features = images_rdd.flatMap(lambda x: x['features'])
    ...
    data_to_cluster = images_rdd.map(lambda x: [x, clusterCenters, pooling])

    #data_to_cluster.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
    #data_to_cluster.persist(pyspark.StorageLevel.MEMORY_AND_DISK_SER)
    #data_to_cluster.persist(pyspark.StorageLevel.MEMORY_ONLY_SER)
    #data_to_cluster.persist(pyspark.StorageLevel.MEMORY_ONLY)
    ...
    print "SUCCESS: Images data processed in {} seconds".format(round(processing_end_time, 3))
